...
function applyColorsToDocument(colors: ThemeColors, isDark: boolean) {
  ...
  // Use preset colors if using a preset, otherwise use custom colors
  const isCustom = colors.preset === "custom";
  const sidebar = isCustom ? colors.sidebar : presetColors.sidebar;
  const pages = isCustom ? colors.pages : presetColors.pages;
  const boardBackground = isCustom
    ? colors.boardBackground
    : presetColors.boardBackground;
